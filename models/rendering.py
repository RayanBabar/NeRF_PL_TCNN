...
def sample_pdf(bins, weights, N_importance, det, eps=1e-5):
    """
    ...
    if det:
        u = torch.linspace(0., 1., N_importance, device=weights.device)
...
def render_rays(models,
    ...
    def inference(model, embedding_xyz, xyz_, dir_, dir_embedded, z_vals, weights_only=False):
        ...
        rgb_final = torch.sum(weights.unsqueeze(-1)*rgb, -2) # (N_rays, 3)
    ...
    if N_importance > 0: # sample points for fine model
        z_vals_mid = 0.5 * (z_vals[:, :-1] + z_vals[:, 1:]) # (N_rays, N_samples-1) interval mid points
        z_vals_ = sample_pdf(z_vals_mid, weights_coarse[:, 1:-1], N_importance, det=(perturb==0)).detach()
